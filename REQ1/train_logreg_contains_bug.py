"""
Bug-Inducing Commit Classifier — Logistic Regression (numeric features only)
=============================================================================
...
Outputs
-------
- Trained model saved to  models/logreg_bug_classifier.joblib
- Console tables:
...
def train(train_df: pd.DataFrame) -> LogisticRegression:
    """Fit a LogisticRegression on the numeric feature matrix."""
...
def predict(clf: LogisticRegression, df: pd.DataFrame) -> np.ndarray:
...
def feature_coefficient_report(clf: LogisticRegression):
    """
    Print feature coefficients sorted by absolute magnitude.
    Positive coefficient → pushes toward bug-inducing (1).
    Negative coefficient → pushes toward clean (0).
    """
    print("=" * 60)
    print("FEATURE COEFFICIENTS (|coef| descending)")
    print("=" * 60)
    coefs = clf.coef_[0]
...
def stratified_report(test_df: pd.DataFrame):
    ...
    out_path = MODEL_DIR / "stratified_results_logreg.csv"
...
def main():
    print("=" * 60)
    print("BUG-INDUCING COMMIT CLASSIFIER — LOGISTIC REGRESSION")
    print("=" * 60)
    ...
    dump(clf, MODEL_DIR / "logreg_bug_classifier.joblib")
    print(f"[SAVE] Model saved to {MODEL_DIR}/logreg_bug_classifier.joblib\n")

    test_df["pred"] = predict(clf, test_df)
    overall_report(test_df["label"].values, test_df["pred"].values)
    feature_coefficient_report(clf)
